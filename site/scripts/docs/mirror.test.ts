...
test(
  'the threat model mirrors its source byte for byte, once it exists',
  ...
  () => {
    ...
    assert.equal(body, readFileSync(join(HERE, '../../../THREATMODEL.md'), 'utf8'))
  },
)

test(
  'related work mirrors its source byte for byte, once it exists',
  ...
  () => {
    ...
    const body = mirrored.split('do not edit -->\n\n')[1]
    assert.equal(body, readFileSync(join(HERE, '../../../RELATED-WORK.md'), 'utf8'))
  },
)

test(
  "the README's limitations section mirrors byte for byte, once it exists",
